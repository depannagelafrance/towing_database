...
DROP PROCEDURE IF EXISTS R_UPDATE_USER_PASSWORD $$
...
CREATE PROCEDURE R_UPDATE_USER_PASSWORD(IN p_login VARCHAR(255), IN p_password VARCHAR(255), IN p_token VARCHAR(255))
BEGIN 
	DECLARE v_company_id BIGINT;
	DECLARE v_user_id VARCHAR(36);
	...
	IF v_user_id IS NULL OR v_company_id IS NULL THEN
		CALL R_NOT_AUTHORIZED;
	ELSE
		UPDATE T_USER_PASSWORDS
        SET pwd = PASSWORD(p_password)
