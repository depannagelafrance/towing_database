...
DROP PROCEDURE IF EXISTS R_FETCH_USER_SIGNATURE $$
...
BEGIN
	...
	ELSE
		...
		IF v_guid IS NULL THEN
			...
			INSERT INTO `T_USERS` (`id`, `company_id`, `login`, `first_name`, `last_name`, `email`, is_towing, is_signa, vehicle_id, `is_active`, `is_locked`, `locked_ts`, `cd`, `cd_by`, `dd`, `dd_by`) 
			VALUES (v_guid, v_company_id, p_login, p_firstname, p_lastname, p_email, p_is_towing, p_is_signa, p_vehicule_id, 1, 0, NULL, now(), F_RESOLVE_LOGIN(v_user_id, p_token),  NULL, NULL);
...
BEGIN
	...
	IF v_user_id IS NULL OR v_company_id IS NULL THEN
		CALL R_NOT_AUTHORIZED;
	ELSE
		SELECT 	`id`, `login`, `first_name`, `last_name`, `email`, is_signa, is_towing, `is_active`, `is_locked`, `locked_ts`
		FROM 	T_USERS
		WHERE 	1 = 1
				AND company_id = v_company_id
				AND dd IS NULL
...
CREATE PROCEDURE R_FETCH_USER_SIGNATURE(IN p_user_id VARCHAR(36), IN p_token VARCHAR(255))
BEGIN
	DECLARE v_company_id BIGINT;
	DECLARE v_user_id VARCHAR(36);
	DECLARE v_doc_id, v_blob_id BIGINT;
...
DROP TRIGGER IF EXISTS TRG_AI_USER $$
DROP TRIGGER IF EXISTS TRG_AU_USER $$

CREATE TRIGGER `TRG_AI_USER` AFTER INSERT ON `T_USERS`
FOR EACH ROW
BEGIN
	CALL R_ADD_USER_AUDIT_LOG(NEW.id);
END $$

CREATE TRIGGER `TRG_AU_USER` AFTER UPDATE ON `T_USERS`
FOR EACH ROW
BEGIN
	CALL R_ADD_USER_AUDIT_LOG(NEW.id);
END $$
