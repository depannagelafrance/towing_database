...
BEGIN
    ...
    SELECT 	customer_number, type, LEFT(company_vat, 2) != 'BE', company_vat
...
-- 				AND tac.towing_voucher_id=p_voucher_id; 
		...
		IF v_has_collector THEN
			...
			IF (SELECT `type` FROM T_CUSTOMERS WHERE id = v_collector_id) != 'CUSTOMER' 
...
BEGIN
	...
	INTO 	v_company_name, v_company_vat, v_street, v_street_number, v_street_pobox, v_zip, v_city, 
            ...
            v_collector_id
    FROM 	T_CUSTOMERS i, T_TOWING_VOUCHERS tv, T_DOSSIERS d
    WHERE tv.id = p_voucher_id
			AND tv.dossier_id = d.id
...
BEGIN
    ...
    THEN
		...
		INTO   v_company_name, v_company_vat, v_street, v_street_number, v_street_pobox, v_zip, v_city, v_country,
			   ...
			   v_voucher_number,
               v_collector_type
		FROM   T_CUSTOMERS c, T_TOWING_VOUCHERS tv, T_DOSSIERS d
		WHERE  tv.id = p_voucher_id
				AND tv.dossier_id = d.id
...
BEGIN
	...
	IF v_collector_id IS NULL 
		OR (v_collector_id IS NOT NULL AND (SELECT `type` FROM T_CUSTOMERS WHERE id = v_collector_id) = 'CUSTOMER')
        OR (v_collector_id IS NOT NULL AND v_company_vat IS NOT NULL AND TRIM(v_company_vat) != '' AND (SELECT lower(`company_vat`) FROM T_CUSTOMERS WHERE id = v_collector_id) = lower(v_company_vat)) 
	THEN
		-- CREATE THE INVOICE LINES FOR THE CREATE INVOICE
		INSERT INTO T_INVOICE_LINES(invoice_id,
									item,
