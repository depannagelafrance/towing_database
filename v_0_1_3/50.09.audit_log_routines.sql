...
DROP PROCEDURE IF EXISTS R_ADD_INVOICE_AUDIT_LOG $$
DROP PROCEDURE IF EXISTS R_ADD_INVOICE_CUSTOMER_AUDIT_LOG $$
DROP PROCEDURE IF EXISTS R_ADD_INVOICE_LINE_AUDIT_LOG $$
...
CREATE PROCEDURE  R_ADD_INVOICE_AUDIT_LOG(p_id BIGINT)
deterministic
BEGIN
	INSERT INTO `AUDIT_P_towing_be`.`T_INVOICES`
	SELECT * FROM T_INVOICES WHERE id = p_id;
END $$

CREATE PROCEDURE  R_ADD_INVOICE_CUSTOMER_AUDIT_LOG(IN p_id BIGINT)
deterministic
BEGIN
	INSERT INTO `AUDIT_P_towing_be`.`T_INVOICE_CUSTOMERS`
	SELECT * FROM T_INVOICE_CUSTOMERS WHERE id = p_id;
END $$

CREATE PROCEDURE  R_ADD_INVOICE_LINE_AUDIT_LOG(IN p_id BIGINT)
deterministic
BEGIN
	INSERT INTO `AUDIT_P_towing_be`.`T_INVOICE_LINES`
	SELECT * FROM T_INVOICE_LINES WHERE id = p_id;
END $$
