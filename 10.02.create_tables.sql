...
CREATE TABLE IF NOT EXISTS `P_MODULE_ROLES` (
  `P_ROLES_id` INT NOT NULL,
  `P_MODULES_id` INT NOT NULL,
  PRIMARY KEY (`P_ROLES_id`, `P_MODULES_id`),
  CONSTRAINT `fk_P_MODULE_ROLES_P_ROLES1`
    FOREIGN KEY (`P_ROLES_id`)
    REFERENCES `P_ROLES` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_P_MODULE_ROLES_P_MODULES1`
    FOREIGN KEY (`P_MODULES_id`)
    REFERENCES `P_MODULES` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;

CREATE INDEX `fk_P_MODULE_ROLES_P_MODULES1_idx` ON `P_MODULE_ROLES` (`P_MODULES_id` ASC);
