...
DROP TRIGGER IF EXISTS TRG_AI_COMPANY_VEHICLE $$
DROP TRIGGER IF EXISTS TRG_AU_COMPANY_VEHICLE $$

CREATE TRIGGER `TRG_AI_COMPANY_VEHICLE` AFTER INSERT ON `T_COMPANY_VEHICLES`
FOR EACH ROW
BEGIN
	CALL R_ADD_COMPANY_VEHICLE_AUDIT_LOG(NEW.id);
END $$

CREATE TRIGGER `TRG_AU_COMPANY_VEHICLE` AFTER UPDATE ON `T_COMPANY_VEHICLES`
FOR EACH ROW
BEGIN
	CALL R_ADD_COMPANY_VEHICLE_AUDIT_LOG(NEW.id);
END $$
