...
DROP TRIGGER IF EXISTS TRG_AI_HOLIDAY $$
DROP TRIGGER IF EXISTS TRG_AU_HOLIDAY $$

CREATE TRIGGER `TRG_AI_HOLIDAY` AFTER INSERT ON `P_HOLIDAYS`
FOR EACH ROW
BEGIN
	CALL R_ADD_HOLIDAY_AUDIT_LOG(NEW.id);
END $$

CREATE TRIGGER `TRG_AU_HOLIDAY` AFTER UPDATE ON `P_HOLIDAYS`
FOR EACH ROW
BEGIN
	CALL R_ADD_HOLIDAY_AUDIT_LOG(NEW.id);
END $$
